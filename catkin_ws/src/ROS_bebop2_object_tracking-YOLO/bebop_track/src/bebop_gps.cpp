...
#include <math.h>
#define ANGULAR_SPEED 3.0
#define DOWN_ANGULAR_SPEED 0.3
#define DOWN_AN_SPEED_GAP 0.5
...
{   
    ...
    while(_isGoHome) // h/H 키가 눌렸으면
    {
        ...
        if(calDistance(home_position.latitude, home_position.longitude, current_position.latitude, current_position.longitude) > 4.0)
        {          
            _bebopControlMessage.linear.x = calDistance(home_position.latitude, home_position.longitude, current_position.latitude, current_position.longitude)/5.0;
...
void BebopGlobalPositioningSystem::_currentGoEnteredCoordinatesCallback(const bebop_msgs::Ardrone3PilotingStatePositionChanged& currentGPS)
{
    ...
    while(_isGoEnteredCoordinates)
    {
        ...
        if(target_angle_set)
        {
            ...
            if((go_latitude < current_position.latitude) && (go_longitude < current_position.longitude)) // 목표위치 기준 1사분면이면
            {
                if(0.0 < bebopAttitude.yaw && bebopAttitude.yaw <= (1.0/2.0)*M_PI) // yaw state 1
                {
                    ...
                    if(target_angle - fabs(bebopAttitude.yaw) < DOWN_AN_SPEED_GAP)
                    {
                ...
                else if(-M_PI < bebopAttitude.yaw && bebopAttitude.yaw <= -(1.0/2.0)*M_PI) // yaw state 3
                {
                    ...
                    {
                        if(target_angle + bebopAttitude.yaw < DOWN_AN_SPEED_GAP)
                        {
            ...
            else if((go_latitude < current_position.latitude) && (go_longitude > current_position.longitude))//목표위치 기준 2사분면이면
            {
                ...
                else if((1.0/2.0)*M_PI < bebopAttitude.yaw && bebopAttitude.yaw <= M_PI) // yaw state 4
                {
                    ...
                    {
                        if(bebopAttitude.yaw - target_angle < DOWN_AN_SPEED_GAP)
                        {
            ...
            else if((go_latitude > current_position.latitude) && (go_longitude < current_position.longitude))//목표위치 기준 4사분면이면
            {
                ...
                {   
                    ...
                    {  
                        if(fabs(bebopAttitude.yaw) - target_angle < DOWN_AN_SPEED_GAP)
                        {
                            _bebopControlMessage.angular.z = -DOWN_ANGULAR_SPEED; // 오른쪽 회전
                            _bebopControlFromGPSPublisher.publish(_bebopControlMessage);
                            ros::spinOnce();
        ...
        //회전 후 직진
        if(calDistance(go_latitude, go_longitude, current_position.latitude, current_position.longitude) > 4.0)
        {
            _bebopControlMessage.linear.x = calDistance(go_latitude, go_longitude, current_position.latitude, current_position.longitude)/15.0;   
            _bebopControlFromGPSPublisher.publish(_bebopControlMessage);
